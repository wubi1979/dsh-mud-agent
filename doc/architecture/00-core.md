...
| **挂起（suspend）** | 命令发出后等结果的状态：实现就是**桥的一条 pending 应答**，结果由判据命中 / GA 帧 / 放弃（原"超时"，§8.4）给出 |
...
**数据流一句话**：`文本块 → 行 → 帧提交（§8.2 消费链：状态折叠 / 规则触发 / 事务结算 / 流程判据 / 投递视图） → 投递消息（动作请求 或 批次） → 官方 agent 回合 → 工具调用 → 桥（挂起/唤醒）`。
