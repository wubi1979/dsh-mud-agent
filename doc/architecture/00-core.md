...
| **帧（frame）** | **消费/提交单位**（v0.6.0，§8）：分帧器（`FrameSplitter`，唯一边界裁决者）以 GA/EOR（八成）与武装判据标记（十成）切帧，帧内存阀兜底防 OOM；状态折叠、规则触发、事务结算、流程判据、投递都是帧提交的订户（§8.2 消费链），**不得自造边界** |
...
入站逻辑行（telnet parsed: GA/EOR/300ms 静默装配/断线；游戏输出与工具应答同源）
      ...
      ▼
┌──────────────────────────────────────────────────────┐
│ 分帧器 FrameSplitter (v0.6.0 §8, 唯一边界裁决者)       │
