...
| 会话侧数据源 | `ctx.mud.agentKit()`：`{ prompt{persona,skillsText,commands}, tools(sessionId), tierNote(sessionId), noteToolCall(sessionId,…), channel(sessionId) }` —— 共享组装与 per-session 状态（队列/在途窗口/world/凭据/投递通道）之间的唯一接法：工具声明共享，执行体按调用方 `agent.id` 解析；`channel` 是 v0.11.0 的投递通道接线口（`beginToolCall`/`endToolCall`/`takeDeferredDeliveries`/`noteToolResult`，§19.6.2） |
...
| 零发送通路 | 新工具 `mud_state`（**world 快照 + 连接态**；不碰 socket、**不返回游戏输出历史** —— T2 的上下文就是会话历史本身）、`mud_help`（命令语法按需查询：不带 topic = 分类 + id 索引, topic = 分类/命令 id 给出完整语法）、`mud_captcha`（fullme 取图 + 推前台弹窗；出站围栏）；只读档工具集 = 这几个 + T1 动作通道（`mud_send`/`world_patch`，强制层约束）。**命令目录注入策略**：系统提示只放 `commandsIndexForAgent()` 的索引（分类 + 命令 id，约 10 行），70+ 条完整语法由 `mud_help` 按需取。`mud_recall` 已取消（v0.11.0，§5/§19.7 定案 7） |
