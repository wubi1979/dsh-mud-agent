...
3. **单元**：感知引擎（跨文本块多行状态、holdDelivery、消费边界）、裁决器行流缓冲（边界标记/武装标记/内存阀/装配阀，`tests/frame-splitter.spec.ts`，v0.6.0；v0.9 W7.1 分帧器并入裁决器，spec 改指 `adjudicator.ts` 导出）、在途窗口表（注册/结算/N-GA 关窗/超时放弃/abort/断线/直发延后集成，`tests/response.spec.ts`；v0.6.0 帧并集版测旧桥，v0.9 W7.2 重写为 `InflightWindowTable` 单元测试）、权限（三档 × 动作矩阵，含 T1 动作被 deny）。
