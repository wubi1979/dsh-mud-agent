...
3. **单元**：感知引擎（跨文本块多行状态、holdDelivery、消费边界）、分帧器（边界标记/武装标记/内存阀/装配阀，`tests/frame-splitter.spec.ts`，v0.6.0）、桥（事务结算/取消/活动表，`tests/response.spec.ts` v0.6.0 帧并集版）、权限（三档 × 动作矩阵，含 T1 动作被 deny）。
