...
>
> **追加（2026-09-10 第二轮架构复审）**：§3.9 **机制 D（长程活动登记）** 为**提议 · 未实施**；
> 对应 `REVIEW-V7-ISSUES.md` 的 P1-4、N5（第一轮）与第二轮复审 **R2-3 / R2-5 / R2-8 / R2-11**，
> 并取代原"长程命令 until 规则集"路线。
>
...
3. **失败收束**：**断线** → `turn/end(error)`。**单次应答超时不再直接回合级 error**（修订）：
   ...
   `reject`；任意非超时结算复位。计数器跨请求递增，非回合级——与"按回合计数"差异见 §3.8 偏差 7。
...
2. **超时/静默窗时长**：初值已实现（GA 10s、静默 2s、声明方配套长程 timeout；`bridgeTimeoutMs/
   ...
   升级阈值 N=3 已实现——**按对象序计数**，与"按回合计数"差异见 §3.8 偏差 7。
...
11. **GA / UNTIL 模式分离**（提议，待采纳；与 §3.9 相互引用）：把"声明 GA"与"声明 UNTIL"做成
    **互斥模式**，不再分层混用。配套四条：(a) 模式**显式**声明（如 `await: {mode:'ga'} |
    {mode:'until', regex, timeout?}`），不再是"传了 until 即隐式切换"；(b) UNTIL 模式**消费**
    等待期间到达的 GA（只作帧切分并记账已消费），仅对"收束后迟到 GA"做**有界一次性**回收
    （消除 N11 的无过期全局计数）；(c) 两模式各自兜底——GA 模式：GA/EOR → 静默窗 → 超时；
    UNTIL 模式：正则命中 → 声明超时（无静默窗）；(d) `MAX_FRAME_LINES` 强制结算仅属 UNTIL 模式，
    且**先测 until 再判上限**（修正 N5）。若采纳，§3.9 判定表按其执行。
...
- 长程命令 `until` 规则集（dz / sleep 等首批）→ **由 §3.9 机制 D（长程活动登记）取代**
  （提议未实施；当前仅有 `COMPLETION_UNTIL` 临时自动附 until 的过渡实现）。
- 噪音白名单、紧急预占、分页下沉评估。
- 实录调优：超时/静默窗初值、并行 mud 命令开放、空收尾步实测、步数上限护栏与 DSH guard 边界核对。

### 3.9 机制 D：长程活动登记（提议 · 未实施）

> **状态：提议，未实施**（2026-09-10 第二轮架构复审提出）。产物：`ActivitySpec` + 进程内活动表
> + 收束后单次注入；**不新增引擎、不引入通用状态机**，判断权继续归规则（T1）与模型（T2）。
> 目的：把"长程命令"从**阻塞式 `until` 声明**里搬出来。

**动机（三条实测痛点，抓包 `probe-2026-09-10T10-45-34` 佐证）**

1. **阻塞回合**：`until` 声明会把工具调用（以及整个回合）park 到完成句——dz ≈ 57s。DSH 的
   `guard/timeout-policy` 是**协作式**的（工具不声明 `timeoutMs` 就不会被 host 杀），所以这是
   "合法但空转 57s"，与 §3.2"长程流程 = 目标驱动多回合"的决策相悖。
2. **完成句正则脆弱 + 现状不一致**：`agent/tools.ts` 的 `COMPLETION_UNTIL` 只认首词 `dz`/`sleep`，
   而抓包实际发的是 **`dazuo 10`**（首词 `dazuo`）→ 同一动作两种结算语义（一个等 57s、一个受理即
   返回）；完成句形态漂移（被打断 / 惊醒 / 区域差异）时只能挂满声明超时（缺省 120s）。
3. **中间态与整段注入**：渐进推送（抓包 **56 批 / 57 行 / ≈57s，无 GA 无 prompt**）在观察窗里
   只能靠静默窗收束；活动更长时会被 `MAX_OBSERVE_LINES`(256) 中途强制 flush，把"半个活动"作为
   一条巨大消息注入回合。

**为什么不做通用状态机（决策记录，避免日后重复讨论）**

- 真实状态只有 `running → completed | interrupted | timed-out` + "ticking/quiet"，转移条件全是
  正则 / 静默 / 预算；画状态图收益≈0。
- 会与现有机能抢所有权：`Perceptor` 的多行状态机、观察窗聚合、`world.flags` + `LOGIN_BOUNDARIES`
  阶段表、dead-air / 登录看门狗（后果观察）已各自拥有"这段文本意味着什么"的一部分。
- 无持久化必要：活动断线即丢，由 world flags + 后果观察兜底（与 `logged_in` / `sent_name` 同哲学）。
- 无当前消费者：MUD 单角色单活动，无并发仲裁；多阶段分支应归规则或 T2 规划，而非桥。

**设计：一个期望 + 三个收束条件 + 一次注入**

```ts
/** 长程活动登记（进程内、可丢弃；不持久化）。提议形态，字段名待实现时定稿。 */
interface ActivitySpec {
  kind: string                     // 'dz' | 'sleep' | 'lian' | ...
  cmd: string | RegExp             // 触发命令（匹配即将发送的命令）
  until?: { regex: string }        // 正常完成条件（逐行匹配，复用 P1-2 修复后的语义）
  interrupt?: readonly string[]    // 打断/异常正则（被打断 / 你死了 / 被吵醒 / …）
  quietMs?: number                 // tick 流：末行后静默即视为结束（dz 抓包 ≈2s）
  maxMs: number                    // 预算兜底（dz 90s / sleep 60s），非主边界
  lane?: 't1' | 't2'               // 收束注入的优先通道（缺省走现判类）
  deliver?: { tail: number }       // 注入内容：摘要头 + 末 N 行（缺省 8）
}

interface ActivityState {          // 收束即销毁；< 10 个字段，无转移图
  spec: ActivitySpec
  startedAt: number
  lastLineAt: number
  lines: MudLine[]                 // 有界（独立上限或复用桥的行集表）
  ticks: number
}
```

**收束** = `until 命中 | interrupt 命中 | 静默 quietMs | 超预算 maxMs`（任一即收束；interrupt
标记 `interrupted`）。收束时**一次性**注入一条带摘要头的观察，例：

```
[活动] 打坐完成 (57s, 56 行, 末 8 行)
......你只觉内息在任脉内回荡，起于玉堂穴…
你将运转于全身经脉间的内息收回丹田，深深吸了口气，站了起来。
```

注入走现有判类（T1 规则命中 → 反射；未命中 → T2），因此**收束开新回合**，而不是 park 当前回合。

**所有权划分（不新增引擎）**

| 关注点 | 归属 |
|---|---|
| 何时开活动 | 工具执行层：`mud_send` 发送时按**活动表**（规则表 / Config，取代 `COMPLETION_UNTIL` 硬编码）登记 |
| 收束条件求值 | 桥 / 观察路径：喂行时逐行测 `until` / `interrupt`（与现有 `until` 同一处）+ 静默 + 预算 |
| 收束后交谁 | 现有 `judgeAndInject` 判类（T1 优先）；`spec.lane` 可覆盖 |
| 活动状态 | 进程内 Map；`connect` / `close` / teardown 丢弃（无持久化、无恢复） |
| 异常升级 | `interrupt` 立即收束；持续无进展 → 复用 dead-air / 看门狗式后果观察 |

**与 GA / UNTIL 模式的关系（判定表）**

| 命令形态 | 桥模式 | 活动登记 | 理由 |
|---|---|---|---|
| 常规命令（GA = 回复结束；抓包 21/21） | GA | 无 | 一次往返即得结果 |
| GA 早到 + 渐进推送（dz / sleep / 练功） | **GA** | **有** | 工具立即返回；完成/打断/静默收束后开新回合（符合"长程=多回合"） |
| 必须在**同一回合**拿到完成句（罕见，如 fullme 需本回合验证码 URL） | UNTIL | 无（或退化） | 阻塞换同回合上下文；代价是回合被 park |
| 分页（`== 未完继续 ==`） | GA | **无** | 每页一个 GA、一页一 step（§2.x / P2-4），不要误用活动登记 |

**可解决的问题（对应 `REVIEW-V7-ISSUES.md` 编号）**

| 编号 | 问题 | 机制 D 如何解决 |
|---|---|---|
| **P1-4** | dz/sleep"完成句驱动反射"当前两条通路都被堵（until 依赖 P1-2、观察窗依赖 P1-3） | 活动以 **GA 模式**受理即返回；完成/打断/静默收束后注入 → T1 优先反射，通路不再依赖阻塞 until |
| **R2-3** | `COMPLETION_UNTIL` 键覆盖不全（`dazuo` 缺失）、形态漂移挂满 120s、正则硬编码在工具层 | 活动表取代它：完成/打断条件进规则表 / Config；`maxMs` 降为兜底而非主边界；`dazuo`/`dz` 同一条目 |
| **R2-5**（第一轮 P3-5 / N5 为"上限缺失 / 观察窗规模"） | `MAX_FRAME_LINES` 先判上限再测 until → 完成句在 256 行之后被误判超时 | GA 模式下帧不再累积到完成句（受理即结算），该矛盾消失；若保留 UNTIL 模式，仍需改成"先测 until 再判上限" |
| **R2-8**（对应第一轮 N5） | 观察窗 256 行只控时机不裁内容 → 单条注入可达 ~10KB | `deliver.tail`：摘要头 + 末 N 行，把"整段注入"变成"摘要 + 尾行" |
| **R2-11**（第一轮 P3-2 的残留） | 孤儿 GA 计数无过期，可能吞掉后续真实 GA | 活动期内 GA 只作帧切分并**记账为本请求已消费**；只需"收束后一次性有界回收"，不再依赖无过期全局计数 |
| §3.2 | "长程流程 = 目标驱动多回合"的决策 | 收束开新回合，替代"一个工具调用 park 57s" |
| §3.7-8 | 长程命令 `until` 规则集未配置 | 改为**活动表**配置（`ActivitySpec`） |

**迁移说明**

- `agent/tools.ts` 的 `COMPLETION_UNTIL`（过渡实现）在机制 D 落地后移除；
- `ActionSpec.until` 保留，但语义收窄为"确需同回合完成句"的少数命令；
- 若本轮提议的 **GA/UNTIL 模式分离**未被采纳，机制 D 与 UNTIL 模式存在语义重叠，应**二选一**
  （推荐机制 D：不 park 回合、条件可组合）。
- 分页（P2-4）与短回显命令**不适用**活动登记，保持 GA 默认路径。

**待定 / 未实施范围**

- `quietMs` 与 `deliver.tail` 取值（抓包建议 dz quiet≈2s / tail 8 行）；
- `interrupt` 首批正则集（被打断 / 你死了 / 被吵醒 / 连接关闭），需实录补齐；
- 活动期是否豁免 `MAX_OBSERVE_LINES`（建议：活动缓冲独立上限，观察窗只承接非活动行）；
- 活动表落位（`config/trigger-rules.ts` 同源表 vs `MudAgentConfig` 可配字段）；
- 与机制五-2（紧急预占，未实施）的合并可能：`interrupt` 即"长程场景的提前收束"；
- 是否暴露 `timeoutMs` 给 DSH `guard/timeout-policy`（当前 mud 工具未声明，故不被 host 杀）。

**验收（实施时）**

1. `dz` 与 `dazuo 10` 行为一致：受理即返回、完成句收束、恰好一次注入；
2. 活动期无中间注入：抓包样本 56 批 → **1 条**（摘要头 + 末 8 行）；
3. 构造打断句（如"你被打断了"）→ 立即收束且 T1 规则可反射；
4. 预算耗尽 → 收束并标记 timedOut，回合不被 park；
5. 断线 → 活动丢弃，无定时器 / 缓冲泄漏。
