 ...
 *               { action: "skill", skill: "login" }                激活命名 skill/flow
 ...
 *   - 命中 action:"skill" → 激活对应 flow/skill (如登录流程)
 *   - 未命中 / action:"llm" → 交给 DSH agent (游戏输出注入, LLM 用同一组工具)
 *
 * 规则引擎定位: 战斗 / 应激反射 / 人物状态捕获 / **skill 激活**。**确定性事务
 * 流程 (登录步骤执行) 不在本表, 由流程引擎 flow.ts 负责** —— 本表只留"等不起
 * LLM 延时的分支反射" + "何时激活哪个 skill" 两类确定性知识。
 *
...
export default [
  // ── skill 激活 (决策中心统一调度: 系统就绪 → 未登录 → 激活登录流程) ──
  {
    ...
    description: '未登录 (login:required) → 激活登录流程 (skill: login)',
  },
