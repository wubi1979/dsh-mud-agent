...
export const LOGIN_BOUNDARIES: {
  ...
  /** 推进 — 密码提示 (抓包实证老号前缀 "ID已存在，"; 前缀/裸双形态全收)。 */
  pass: [
    /^(?:ID已存在，)?请输入密码[：:]\s*$/,
  ],
...
const defaultPerceptionRules: readonly PerceptionRule[] = [
  ...
  },
  {
    ...
    priority: 30,
    // 抓包实证 (2026-09-10, 8081, 老号复登): 真实提示为 "ID已存在，请输入密码：" —
    // 旧规则 ^请输入密码 锚定漏匹配 → 密码永不发 → 登录无声卡死。前缀/裸双形态全收。
  ...
  // ── pager 分页自动翻页 (R5, 实测驱动): pkuxkx 大地图/长输出停在
  //    `== 未完继续 NN% == (q 离开，b 前一页，其他继续下一页)` 时, 输入任何
  //    一行都会被当作翻页 (实测: sleep 被吞成翻页, 命令失真)。命中即发空格
  //    翻页, 直到服务器输出完成; 分页期间 agent 不应再发业务命令 (T1 渲染层
  //    自然节流: 一批最多命中一次)。
