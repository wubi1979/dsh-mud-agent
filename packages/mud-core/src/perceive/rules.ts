...
/**
 * 构造一份**全新的**默认规则表 (每个感知引擎 = 每个会话一份)。
 *
 * 为什么是工厂而不是共享常量: `guard` 闭包带**运行态** —— pager:continue 用
 * `lastPageFlipAt` 做 1s 节流。模块级共享同一份数组会让该闭包被**所有会话**共用:
 * 会话 A 的自动翻页会压住会话 B 的翻页 (违反 §1 I8「禁止模块级可变单例状态」);
 * 测试里也表现为"同一份样本喂第二次必然不命中"。数据本身 (正则字面量/文本/优先级)
 * 逐份相同, 只有运行态是各自的。
 */
export function createDefaultPerceptionRules(): readonly PerceptionRule[] {
  /** 上次自动翻页的时间戳 (本份规则表独有 ⇒ 本会话独有)。 */
