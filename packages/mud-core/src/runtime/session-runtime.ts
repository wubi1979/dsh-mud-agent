...
import { buildMudTools, type ActivityEntry, type MudTools, type SessionCredentials } from '../agent/tools.ts'
import type { DangerousRule } from '../config/commands.ts'
import { DEFAULT_DANGEROUS_COMMANDS } from '../config/commands.ts'
...
import { applyPatch, createWorld, worldSnapshot, type WorldModel } from '../world/world.ts'
import { CONTROL_PREFIX } from '../trigger-llm/types.ts'
import type { PerceptionRule } from '../trigger-llm/types.ts'
import { PerceptionEngine, type EngineHit } from '../perception/engine.ts'
import { splitDelivery } from '../perception/split.ts'
import { MudConnectionManager, type MudConnectionSink } from './connection.ts'
import { WatchdogTable } from './watchdogs.ts'
import { FlowRuntime, type FlowActionHit, type FlowState } from './flow-runtime.ts'
import { defaultFlows, type FlowSpec } from '../config/flows.ts'
import type { MudUiItem, MudWorldSnapshot } from '../client/wire.ts'

/** 一条待投递的动作请求 (与投递消息 `source.actions` 同形; §7)。 */
type ActionRequest = OwnedAction
 ...
 * @deprecated 由 `config/flows.ts` 的 `LOGIN_FLOW` 声明取代（`doc/ARCHITECTURE.md` §11/§19）。
 */
 ...
 */
function actionOf(ruleId: string, action: { output: string; tool?: { name: string; args?: Record<string, unknown> } }): ActionRequest {
  return {
    ruleId,
 ...
 */
function parseDeliveryCallId(callId: string): { delivery: string; index: number } | null {
