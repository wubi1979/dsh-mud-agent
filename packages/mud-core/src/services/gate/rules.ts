...
/**
 * 组装缺省 GateRules (从 shared/game + shared/commands)。
 *
 * 命令派生器与 `agents/tools.ts` 的模板口径一致: `mud_move` → 方向全名
