/**
 * dsh-mud-core — 匹配服务契约 (matcher/types): 机制层规则/命中形状。
 *
 * 与策略面 (感知规则、动作目标) 解耦: 感知层把 `PerceptionRule` 投影为
 * `MatcherRule<TAction>` 后交给引擎; 引擎只做**两段式匹配** (预筛 + 准入提取,
 * 含多行 Mudlet 逐条件状态机) 与命中产出, 不理解 action 语义。
 *
 * 匹配两段式:
