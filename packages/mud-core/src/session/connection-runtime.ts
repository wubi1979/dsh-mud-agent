 ...
 *
 * 传输层 (`network/manager.ts`) 仍是会话无关的注册表; 本类是"会话 → 连接"
 * 绑定 (`connectionId`) 的唯一持有者, 方向单向、传输层永不回指。
 * @module @deepseek-ai/dsh-mud-core/session/connection-runtime
 */

import type { MudConnectionManager, MudConnectionSink } from '../network/manager.ts'
...
export interface ConnectionRuntimeOptions {
  ...
  onLines(lines: import('../network/ansi.ts').MudLine[]): void
