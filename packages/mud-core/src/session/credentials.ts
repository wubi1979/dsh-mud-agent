 ...
 * 明文只在发送瞬间插值 (`agent/tools-build.ts` 的 `interpolateCredentials`)。本模块提供
 ...
 * @module @deepseek-ai/dsh-mud-core/session/credentials
 */

// ── 会话登录凭据 (明文最小暴露面) ──────────────
// 凭据由**会话运行时**持有 (ConnectionRuntime.account), 不设模块级共享表:
// 引用一律以 {name}/{pass} 占位符流转 (转录/日志/工具结果均只见占位符),
// 明文仅在 mud_send 发送瞬间插值 (agent/tools-build.ts)。
