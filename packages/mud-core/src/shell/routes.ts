...
export function installRoutes(rc: RouteContext): () => void {
  ...
  /**
   * POST + JSON body 路由样板 (method 校验 + body 解析 + 统一 400)。
   * @param path 路由路径。
   * @param logLabel 解析失败日志前缀 (如 '[SYS] 绑定')。
   * @param handler 业务处理 (body 已解析; 自己负责 sendJson 回包)。
   */
  const jsonPost = (
    path: string,
    logLabel: string,
    handler: (body: Record<string, unknown>, res: ServerResponse) => void | Promise<void>,
  ): (() => void) =>
    createRoute({
      kind: 'exact',
      path,
      handler: (req, res) => {
        ...
        readJsonBody(req).then((body) => handler(body, res)).catch((err: unknown) => {
          tuiLog('', `${logLabel}请求解析失败: ${err instanceof Error ? err.message : String(err)}`)
  ...
  const disposeBindRoute = jsonPost('/mud/bind', '[SYS] 绑定', (body, res) => {
    const sessionId = sessionIdOf(body)
    service.bind(sessionId)
    sendJson(res, 200, { ok: true, sessionId })
  })

  const disposeConnectRoute = jsonPost('/mud/connect', '[SYS] 连接', (body, res) => {
    const sessionId = sessionIdOf(body)
  ...
  const disposeDisconnectRoute = jsonPost('/mud/disconnect', '[SYS] 断开', (body, res) => {
  ...
  const disposeCommandRoute = jsonPost('/mud/command', '[SYS] 命令', (body, res) => {
    const sessionId = sessionIdOf(body)
  ...
  const disposeCaptchaRefreshRoute = jsonPost('/mud/captcha/refresh', '[SYS] 验证码刷新', async (body, res) => {
  ...
  const disposeLogsRoute = jsonPost('/mud/logs', '[SYS] 日志恢复', (body, res) => {
    const sessionId = sessionIdOf(body)
  ...
  const disposePurgeRoute = jsonPost('/mud/purge', '[SYS] 注销', (body, res) => {
  ...
  // /mud/command 的形状一致。GET/POST 混合路由, 无法套 jsonPost 样板, 保持手工分派。
