...
import type { MudDeliveryChannel } from '../session/mount.ts'
import type { MudGameItem, MudWorldSnapshot } from './remote-types.ts'
import type { SkillService } from '../agent/skills.ts'
import type { MudTools } from '../agent/tools-schema.ts'
import type { MudSessionDiag } from '../session/types.ts'
import type { MudCapabilityApi } from '../agent/gate/capability.ts'
import type { MudTier } from '../agent/gate/tiers.ts'
...
/** 单会话诊断 (类型本体在 `session/types.ts`, 随 `MudSessionRuntime.diag()` 声明)。 */
...
export interface MudCoreService {
  ...
  /**
   * preset 线 (`session/preset.ts`) 的装配数据源 (§9)。
   *
