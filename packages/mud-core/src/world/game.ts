/**
 * dsh-mud-core 的游戏命令知识 (world/game): 移动方向别名表、状态查询命令映射。
 *
 * 纯数据模块, 零依赖; 由 `agent/tools-build` 与 `agent/gate` 作为"注入规则"共同消费,
 * 使安全闸门保持机制通用 (规则注入模式, 参见 `agent/gate/policy.ts`)。
 *
 * @module @deepseek-ai/dsh-mud-core/world/game
 */
